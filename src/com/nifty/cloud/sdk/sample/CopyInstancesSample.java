...
import com.nifty.cloud.sdk.server.model.LoadBalancer;
 ...
 *
 */
public class CopyInstancesSample {

    /**
     * CopyInstancesを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、CopyInstancesを実行します。
         ***********************************************************************/
        CopyInstancesRequest request = new CopyInstancesRequest();
        // request.setInstanceId("instanceId");
        // CopyInstance copyInstance = new CopyInstance();
		// copyInstance.setInstanceName("instanceName");
        // copyInstance.setInstanceType("instanceType");
        // copyInstance.setAccountingType("accountingType");
        // copyInstance.setIpType("static");
        ...
        // copyInstance.setLoadBalancers(loadBalancers);
        // List<String> groupIds = new ArrayList<String>();
        // groupIds.add("groupId");
        // copyInstance.setGroupIds(groupIds);
        ...
        // copyInstance.setPlacement(placement);
        // request.setCopyInstance(copyInstance);
        // request.setCopyCount(0);
        invokeCopyInstances(client, request);

    }

    /**
     * CopyInstances 実行サンプル
     * 設定されたリクエストから CopyInstancesを実行し、実行結果を表示します。
     *
     * @param client   クライアント
     * @param request  リクエスト
     */
    public static void invokeCopyInstances(NiftyServerClient client,
            CopyInstancesRequest request) {
        try {
            CopyInstancesResult result = client.copyInstances(request);

            out.println("CopyInstances Action Response");
            out.println("=============================================================================");
            if (result.getCopyInstances() != null) {
                List<CopyInstanceSet> copyInstances = result.getCopyInstances();
                for (CopyInstanceSet copyInstanceSet : copyInstances) {
                    out.println("CopyInstances");
                    if (copyInstanceSet != null) {
                        if (copyInstanceSet.getInstanceId() != null) {
                            out.println("  InstanceId     : " + copyInstanceSet.getInstanceId());
                        }
                        if (copyInstanceSet.getInstanceState() != null) {
                            out.println("  InstanceState  : " + copyInstanceSet.getInstanceState());
                        }
                    }
                }
            }
