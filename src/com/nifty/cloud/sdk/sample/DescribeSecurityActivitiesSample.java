...
import com.nifty.cloud.sdk.firewall.model.SecurityActivityRange;

/**
 * DescribeSecurityActivitiesサンプル
 *
 */
public class DescribeSecurityActivitiesSample {

    /**
     * DescribeSecurityActivitiesを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、DescribeSecurityActivitiesを実行します。
         ***********************************************************************/
        DescribeSecurityActivitiesRequest request = new DescribeSecurityActivitiesRequest();
        // request.setGroupName("groupName");
        // request.setActivityDate("yyyymmdd");
        // SecurityActivityRange securityActivityRange = new SecurityActivityRange();
        // securityActivityRange.setAll(false);
        // securityActivityRange.setStartNumber(1);
        // securityActivityRange.setEndNumber(100);
        // request.setRange(securityActivityRange);

        invokeDescribeSecurityActivities(client, request);
    }

    /**
     * DescribeSecurityActivities 実行サンプル
     * 設定されたリクエストから DescribeSecurityActivitiesを実行し、実行結果を表示します。
     *
     * @param client   クライアント
     * @param request  リクエスト
     */
    public static void invokeDescribeSecurityActivities(
            NiftySecurityGroupClient client,
            DescribeSecurityActivitiesRequest request) {
        try {
            DescribeSecurityActivitiesResult result = client.describeSecurityActivities(request);

            out.println("DescribeSecurityActivities Action Response");
            out.println("=============================================================================");

            if (result.getGroupName() != null) {
                out.println("  GroupName  : " + result.getGroupName());
            }
            if (result.getLog() != null) {
                List<String> logs = new ArrayList<String>(Arrays.asList(result.getLog().split("\n")));
                out.println("  Log        : " + logs.remove(0));
                if (!logs.isEmpty()) {
                    for (String log : logs) {
                        out.println("               " + log);
                    }
