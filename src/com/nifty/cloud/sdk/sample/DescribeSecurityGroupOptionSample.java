...
/**
 * DescribeSecurityGroupOptionサンプル
 *
 */
public class DescribeSecurityGroupOptionSample {

    /**
     * DescribeSecurityGroupOptionを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、DescribeSecurityGroupOptionを実行します。
         ***********************************************************************/
        DescribeSecurityGroupOptionRequest request = new DescribeSecurityGroupOptionRequest();

        invokeDescribeSecurityGroupOption(client, request);
    }

    /**
     * DescribeSecurityGroupOption 実行サンプル
     * 設定されたリクエストから DescribeSecurityGroupOptionを実行し、実行結果を表示します。
     * 
     * @param client   クライアント
     * @param request  リクエスト
     */
    public static void invokeDescribeSecurityGroupOption(
            NiftySecurityGroupClient client,
            DescribeSecurityGroupOptionRequest request) {
        try {
            DescribeSecurityGroupOptionResult result = client.describeSecurityGroupOption(request);

            out.println("DescribeSecurityGroupOption Action Response");
            out.println("=============================================================================");

            if (result.getCourse() != null) {
                out.println("Course              : " + result.getCourse());
            }
            if (result.getSecurityGroupLimit() != null) {
                out.println("SecurityGroupLimit  : " + result.getSecurityGroupLimit());
            }
