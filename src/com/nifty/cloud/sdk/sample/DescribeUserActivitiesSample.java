...
/**
 * DescribeUserActivitiesサンプル
 *
 */
public class DescribeUserActivitiesSample {

    /**
     * DescribeUserActivitiesを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、DescribeUserActivitiesを実行します。
         ***********************************************************************/
        DescribeUserActivitiesRequest request = new DescribeUserActivitiesRequest();
        
         // request.setYearMonth("yearMonth");
        
        invokeDescribeUserActivities(client, request);
    }

    /**
     * DescribeUserActivities 実行サンプル。
     * 設定されたリクエストから DescribeUserActivities を実行し、実行結果を表示します。
     * 
     * @param client クライアント
     * @param request リクエスト
     */
    public static void invokeDescribeUserActivities(NiftyUsageClient client,
            DescribeUserActivitiesRequest request) {
        try {
            DescribeUserActivitiesResult result = client.describeUserActivities(request);
            
            out.println("DescribeUserActivities Action Response");
            out.println("=============================================================================");
            
            if (result.getUserActivities() != null) {
                out.println("UserActivities");
                List<UserActivity> userActivities = result.getUserActivities();
                for (UserActivity userActivity : userActivities) {
                    if (userActivity.getDateTime() != null) {
                        out.println("  DateTime     : " + userActivity.getDateTime());
                    }
                    if (userActivity.getIpAddress() != null) {
                        out.println("  IpAddress    : " + userActivity.getIpAddress());
                    }
                    if (userActivity.getCategoryName() != null) {
                        out.println("  CategoryName : " + userActivity.getCategoryName());
                    }
                    if (userActivity.getOperator() != null) {
                        out.println("  Operator     : " + userActivity.getOperator());
                    }
                    if (userActivity.getOperation() != null) {
                        out.println("  Operation    : " + userActivity.getOperation());
                    }
                    if (userActivity.getResult() != null) {
                        out.println("  Result       : " + userActivity.getResult());
                    }
                }
            }
            out.println("=============================================================================");
