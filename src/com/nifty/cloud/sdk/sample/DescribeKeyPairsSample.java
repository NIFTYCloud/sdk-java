...
/**
 * DescribeKeyPairsサンプル
 *
 */
public class DescribeKeyPairsSample {

    /**
     * DescribeKeyPairsを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、DescribeKeyPairsを実行します。
         ***********************************************************************/
        DescribeKeyPairsRequest request = new DescribeKeyPairsRequest();
        // List<String> keyNames = new ArrayList<String>();
        // keyNames.add("keyName");
        // request.setKeyNames(keyNames);
        invokeDescribeKeyPairs(client, request);

    }

    /**
     * DescribeKeyPairs 実行サンプル
     * 設定されたリクエストから DescribeKeyPairs　を実行し、実行結果を表示します。
     *
     * @param client  クライアント
     * @param request リクエスト
     */
    public static void invokeDescribeKeyPairs(NiftySecurityClient client,DescribeKeyPairsRequest request) {
        try {
            DescribeKeyPairsResult result = client.describeKeyPairs(request);

            out.println("DescribeKeyPairs Action Response");
            out.println("=============================================================================");

            if (result.getKeyPairs() != null) {
                List<KeyPairInfo> keyPairInfos = result.getKeyPairs();
                for (KeyPairInfo keyPairInfo : keyPairInfos) {
                    out.println("KeyPairInfo");
                    if (keyPairInfo.getKeyName() != null) {
                        out.println("  KeyName        : " + keyPairInfo.getKeyName());
                    }
                    if (keyPairInfo.getKeyFingerprint() != null) {
                        out.println("  KeyFingerprint : " + keyPairInfo.getKeyFingerprint());
                    }
                }
            }
            out.println("=============================================================================");
