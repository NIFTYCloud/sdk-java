...
/**
 * ConfigureHealthCheckサンプル
 *
 */
public class ConfigureHealthCheckSample {

    /**
     * ConfigureHealthCheckを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、ConfigureHealthCheckを実行します。
         ***********************************************************************/
        ...
        // healthCheck.setInterval(0);
        // healthCheck.setTimeout(30);
        // healthCheck.setUnhealthyThreshold(1);
        // healthCheck.setHealthyThreshold(1);
        // request.setHealthCheck(healthCheck);
        invokeConfigureHealthCheck(client, request);

    }

    /**
     * ConfigureHealthCheck 実行サンプル
     * 設定されたリクエストから ConfigureHealthCheck を実行し、実行結果を表示します。
     *
     * @param client  クライアント
     * @param request リクエスト
     */
    public static void invokeConfigureHealthCheck(
            NiftyLoadBalancerClient client, ConfigureHealthCheckRequest request) {
        try {
            ConfigureHealthCheckResult result = client.configureHealthCheck(request);

            out.println("HealthCheck Action Response");
            out.println("=============================================================================");
            if (result.getHealthCheck() != null) {
                out.println("HealthCheck");
                HealthCheck healthCheck = result.getHealthCheck();
