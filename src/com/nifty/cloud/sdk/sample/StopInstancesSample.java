 ...
 *
 */
public class StopInstancesSample {

    /**
     * StopInstancesを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、StopInstancesを実行します。
         ***********************************************************************/
        ...
        invokeStopInstances(client, request);

    }

    /**
     * StopInstances 実行サンプル
     * 設定されたリクエストから StopInstances を実行し、実行結果を表示します。
     *
     * @param client  クライアント
     * @param request リクエスト
     */
    public static void invokeStopInstances(NiftyServerClient client,
            StopInstancesRequest request) {
        try {
            StopInstancesResult result = client.stopInstances(request);
            out.println("StopInstances Action Response");
            out.println("=============================================================================");
            if (result.getStoppingInstances() != null) {
                ...
                for (InstanceStateChange stoppingInstance : stoppingInstances) {
                    out.println("StoppingInstances     ");
                    if (stoppingInstance.getInstanceId() != null) {
                        out.println("  InstanceId        : " + stoppingInstance.getInstanceId());
                    }
                    if (stoppingInstance.getCurrentState() != null) {
                        InstanceState currentState = stoppingInstance.getCurrentState();
                    ...
                    if (stoppingInstance.getPreviousState() != null) {
                        InstanceState previousState = stoppingInstance.getPreviousState();
