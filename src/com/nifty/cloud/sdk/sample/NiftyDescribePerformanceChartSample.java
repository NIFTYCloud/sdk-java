...
/**
 * NiftyDescribePerformanceChartサンプル
 *
 */
public class NiftyDescribePerformanceChartSample {

    /**
     * NiftyDescribePerformanceChartを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、NiftyDescribePerformanceChartを実行します。
         ***********************************************************************/
        NiftyDescribePerformanceChartRequest request = new NiftyDescribePerformanceChartRequest();
        ...
        invokeNiftyDescribePerformanceChart(client, request);

    }

    /**
     * NiftyDescribePerformanceChart 実行サンプル
     * 設定されたリクエストから NiftyDescribePerformanceChart　を実行し、実行結果を表示します。
     *
     * @param client   クライアント
     * @param request  リクエスト
     */
    public static void invokeNiftyDescribePerformanceChart(NiftyMiscClient client,
            NiftyDescribePerformanceChartRequest request) {
        try {
            NiftyDescribePerformanceChartResult result = client.niftyDescribePerformanceChart(request);

            out.println("NiftyDescribePerformanceChart Action Response");
            out.println("=============================================================================");

            if (result.getFunctionName() != null) {
                out.println("FunctionName     : " + result.getFunctionName());
            }
            if (result.getValueType() != null) {
                out.println("ValueType        : " + result.getValueType());
            }
            if (result.getPerformanceCharts() != null) {
                List<PerformanceChart> performanceCharts = result.getPerformanceCharts();
                for (PerformanceChart performanceChart : performanceCharts) {
                    out.println("PerformanceChart");
                    if (performanceChart.getResourceName() != null) {
                        out.println("  ResourceName   : " + performanceChart.getResourceName());
                    }
                    if (performanceChart.getDataType() != null) {
                        out.println("  DataType       : " + performanceChart.getDataType());
                    }
                    if (performanceChart.getDatas() != null) {
                        List<Data> datas = performanceChart.getDatas();
                        for (Data data : datas) {
                            out.println("  Data");
                            if (data.getDateTime() != null) {
                                out.println("    DateTime     : " + data.getDateTime());
                            }
                            if (data.getValue() != null) {
                                out.println("    Value        : " + data.getValue());
                            }
                        }
                    }
                }
