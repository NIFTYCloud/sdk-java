...
public class DescribeRegionsSample {
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、DescribeRegionsを実行します。
         ***********************************************************************/
        DescribeRegionsRequest request = new DescribeRegionsRequest();
        // List<String> regionNames = new ArrayList<String>();
        // regionNames.add("regionName");
        // request.setRegionNames(regionNames);
        invokeDescribeRegions(client, request);

    }

    /**
     * DescribeRegions 実行サンプル 設定されたリクエストから
     * DescribeRegions　を実行し、実行結果を表示します。
     *
     * @param client  クライアント
     * @param request リクエスト
     */
    public static void invokeDescribeRegions(NiftyMiscClient client,
            DescribeRegionsRequest request) {
        try {
            DescribeRegionsResult result = client.describeRegions(request);

            out.println("DescribeRegions Action Response");
            out.println("=============================================================================");
            ...
            if (result.getRegions() != null) {
                ...
                for (Region region : regions) {
                	...
                	if (region.getRegionName() != null) {
                		out.println("  RegionName     : " + region.getRegionName());
                	}
                	if (region.getRegionEndpoint() != null) {
                		out.println("  RegionEndpoint : " + region.getRegionEndpoint());
                	}
                    if (region.getMessages() != null) {
                        List<String> messages = region.getMessages();
                    ...
                    if (region.getIsDefault() != null) {
                    	out.println("  IsDefault      : " + region.getIsDefault());
                    }
                }
            }
            out.println("=============================================================================");
