...
/**
 * AssociateAddressサンプル
 *
 */
public class AssociateAddressSample {

    /**
     * AssociateAddressを実行するサンプルです。
    ...
    public static void main(String[] args) {
        ...
        /************************************************************************
         * リクエストパラメーターを設定し、AssociateAddressを実行します。
         ***********************************************************************/
         ...
         // request.setNiftyReboot("force");
         // request.setAllocationId("allocationId");
         // request.setNetworkInterfaceId("networkInterfaceId");
         // request.setAllowReassociation(true);
        invokeAssociateAddress(client, request);
    }

    /**
     * AssociateAddress 実行サンプル
     * 設定されたリクエストから AssociateAddress を実行し、実行結果を表示します。
     *
     * @param client   クライアント
     * @param request  リクエスト
     */
    public static void invokeAssociateAddress(NiftyAddressClient client,
            AssociateAddressRequest request) {
        try {
            AssociateAddressResult result = client.associateAddress(request);

            out.println("AssociateAddress Action Response");
            out.println("=============================================================================");

            if (result.getReturn() != null) {
                out.println("Return          : " + result.getReturn());
            }
            if (result.getAssociationId() != null) {
                out.println("AssociationId   : " + result.getAssociationId());
            }
