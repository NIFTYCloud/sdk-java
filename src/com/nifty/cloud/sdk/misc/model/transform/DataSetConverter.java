...
/**
 * PerformanceChartSetType変換クラス。<br />
 * このクラスはレスポンスXMLのdataSet要素からDataクラスのリストを生成します。
 */
public class DataSetConverter implements Converter {
	...
	/**
	 * {@literal
	 * dataSet要素をList<Data>クラスに変換します。}
	 *
	 * @param reader XML読込リーダ
	 * @param context
	 * @return {@literal List<Data>オブジェクト}
	 *
		...
		while(reader.hasMoreChildren()) {
			...
			while (reader.hasMoreChildren()) {
				...
				if (name.equals(ELEMENT_DATE_TIME)) {
					data.setDateTime(reader.getValue());
				}
				else if (name.equals(ELEMENT_VALUE)) {
					data.setValue(new Integer(reader.getValue()));
				}
				reader.moveUp();
			}
			datas.add(data);
